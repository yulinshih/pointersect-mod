...
from plib import rigid_motion, render, pixel2world
...
def generate_camera_rays_from_uv(
    ...
    if not pano:
        ...
        assert _2 == 2
        print("[pano]")
        cam_pose = cam_poses[:, :3, 3]
        ray_origins_w = cam_poses[..., :3, 3].clone().expand(m, *p_shape, 3)
        ray_directions_w = pixel2world.pixel2world(cam_pose, width_px, height_px)
        ray_directions_w = ray_directions_w / torch.linalg.vector_norm(
            ray_directions_w, dim=-1, keepdims=True)  # (m, *p, 3)
